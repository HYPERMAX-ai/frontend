<template>
  <wmx-modal container-class="lending-market-borrow-modal pt-[16px] pb-0 px-0" v-model="modelValue">
    <template #title>
      <wmx-title class="px-[24px]" :show-refresh="false">{{ selectSupply === 0 ? 'Borrow' : 'Repay' }}</wmx-title>
    </template>
    <template #default>
      <div class="flex flex-col gap-[24px] w-full tablet:w-[608px] px-[24px] pb-[24px]">
        <wmx-select-bar :items="['Borrow', 'Repay']" :options="{
          handle: {
            text: 'text-[var(--color-btn-active2-text)]',
            bg: 'bg-[var(--color-btn-active2-bg)]',
          },
        ...
        <transaction-setting
            ...
            :options="{
              percentSelector: {
                handleColor: 'var(--color-title-text)',
                handleTextColor: 'var(--color-btn-active2-text)'
              }
            }"
        ...
        <receipt-panel class="h-[131px] flex flex-col justify-end">
          <receipt-panel-item v-if="selectSupply === 0" label="Borrow APY" value="12%" class="!font-bold" />
          <receipt-panel-item label="Current Borrow" value="234.1212 > 234.1212"/>
        ...
        <button class="w-full btn-radius bg-[var(--color-btn-active2-bg)] text-[var(--color-btn-active2-text)] font-semibold">
          {{ selectSupply === 0 ? 'Borrow' : 'Repay' }}
        </button>
      </div>
    </template>
  </wmx-modal>
